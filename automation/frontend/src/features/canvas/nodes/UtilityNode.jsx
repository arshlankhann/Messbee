        ...
        <div style={{ padding: '12px', background: '#fafafa' }}>
          ...
          <div style={{ 
            ...
            {data.utilityType === 'location' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ 
                  ...
                  <div style={{ position: 'absolute', bottom: '6px', right: '6px', background: 'rgba(255,255,255,0.8)', padding: '2px 6px', borderRadius: '4px', fontSize: '10px', color: '#6b7280', border: '1px solid #e5e7eb' }}>Live Map</div>
                </div>
                <div>
                  <div style={{ fontSize: '13px', fontWeight: '600', color: '#111827' }}>{data.locationName || 'Location Name'}</div>
                  <div style={{ fontSize: '12px', color: '#6b7280', marginTop: '2px' }}>{data.locationAddress || 'Address details...'}</div>
                </div>
              </div>
            )}
            ...
            {data.utilityType === 'contact' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <div>
                    <div style={{ fontSize: '13px', fontWeight: '600', color: '#111827' }}>{data.contactName || 'John Doe'}</div>
                    <div style={{ fontSize: '12px', color: '#6b7280', marginTop: '2px' }}>{data.contactPhone || '+1 234 567 8900'}</div>
                  </div>
                </div>
                <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: '10px', display: 'flex', justifyContent: 'center', gap: '16px' }}>
            ...
            {data.utilityType === 'calendar' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ display: 'flex', gap: '12px', alignItems: 'center', background: '#f9fafb', padding: '10px', borderRadius: '6px', border: '1px solid #e5e7eb' }}>
                  ...
                  <div>
                    <div style={{ fontSize: '13px', fontWeight: '600', color: '#111827' }}>{data.eventName || 'Event Title'}</div>
                    <div style={{ fontSize: '11px', color: '#6b7280', marginTop: '2px' }}>{data.eventTime || '10:00 AM - 11:00 AM'}</div>
                  </div>
                </div>
              </div>
            )}
            ...
            {data.text && (
              <div style={{ fontSize: '12px', color: '#374151', whiteSpace: 'pre-wrap', lineHeight: '1.4', borderTop: '1px solid #e5e7eb', paddingTop: '10px', marginTop: '10px' }}>
                {data.text}
              </div>
            )}
          </div>
