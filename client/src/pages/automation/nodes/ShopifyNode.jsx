...
import { Copy, Trash2, ShoppingBag, Database, Link as LinkIcon, Settings, AlertTriangle } from 'lucide-react';
...
export default function ShopifyNode({ id, data, selected }) {
  ...
  const shopifyAction = data.shopifyAction || 'get_customer';
  const shopifyStoreUrl = data.shopifyStoreUrl || '';
  const isValid = !!shopifyStoreUrl;
    ...
    <div style={{ position: 'relative', width: '280px', fontFamily: '"Inter", "Outfit", sans-serif' }}>
      ...
      <div style={{
        ...
        <div style={{ padding: '12px', background: '#2E3440' }}>
          ...
          <div style={{ background: '#3B4252', borderRadius: '6px', padding: '12px', border: '1px solid #4C566A', display: 'flex', flexDirection: 'column', gap: '12px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              ...
              <select 
                value={shopifyAction} 
                onChange={(e) => updateNodeData(id, { shopifyAction: e.target.value })}
              ...
              >
                <option value="get_customer">Get Customer by Phone</option>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              ...
              <input 
                ...
                onChange={(e) => updateNodeData(id, { shopifyStoreUrl: e.target.value })}
        ...
        <div style={{ padding: '12px', borderTop: '1px solid #4C566A', background: '#2E3440', position: 'relative' }}>
          ...
          <Handle type="source" position={Position.Right} id="success" className="custom-handle" style={{ right: '-6px', top: '50%', transform: 'translateY(-50%)', background: '#3B4252', border: '2px solid #10B981', width: '12px', height: '12px' }} />
        </div>
